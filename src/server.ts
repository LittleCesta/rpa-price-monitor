  ...
  try {
    ...
    const productsWithPrice = await Promise.all(
      products.map(async (product) => {
        const latest = await PriceHistory.findOne({ productId: product._id })
          .sort({ scrapedAt: -1 })
        ...
        return {
          ...
          belowTarget: currentPrice !== null && currentPrice <= product.targetPrice,
...
app.post('/api/products', async (req, res) => {
  try {
    ...
    if (!name || !url || !targetPrice) {
      return res.status(400).json({ error: 'name, url e targetPrice são obrigatórios' })
    }

    const existing = await Product.findOne({ url })
    if (existing) {
      return res.status(409).json({ error: 'Produto com essa URL já cadastrado' })
    }
